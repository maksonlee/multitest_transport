...
class TestKickerTest(absltest.TestCase):
  ...
  @mock.patch.object(tfc_client, 'NewRequest', autospec=True)
  @mock.patch.object(download_util, 'DownloadResource', autospec=True)
  def testKickTestRun_withResultReportActions(
      self, mock_download_resource, mock_new_request):
    test_run = self._CreateMockTestRun(command='command')
    report_action = ndb_models.ResultReportAction(
        id='foo',
        name='Foo',
        tradefed_result_reporters=[
            ...
            ndb_models.TradefedConfigObject(
                class_name='com.android.bar'
            ),
        ])
    report_action.put()
    test_run.test_run_config.result_report_action_keys = [report_action.key]
    test_run.result_report_actions = [report_action]
    test_run.put()
    test_run_id = test_run.key.id()
    mock_download_resource.return_value = 'cache_url'
    mock_request = api_messages.RequestMessage(id='request_id')
    ...
    # two TF result reporters with right class names and options
    tradefed_config_objects = msg.test_environment.tradefed_config_objects
    self.assertEqual(2, len(tradefed_config_objects))
    self.assertEqual(
        api_messages.TradefedConfigObjectType.RESULT_REPORTER,
        tradefed_config_objects[0].type)
    self.assertEqual(
        report_action.tradefed_result_reporters[0].class_name,
        tradefed_config_objects[0].class_name)
    self.assertEqual(
        report_action.tradefed_result_reporters[0].option_values[0].name,
        tradefed_config_objects[0].option_values[0].key)
    self.assertEqual(
        [str(test_run_id)], tradefed_config_objects[0].option_values[0].values)
    self.assertEqual(
        api_messages.TradefedConfigObjectType.RESULT_REPORTER,
        tradefed_config_objects[1].type)
    self.assertEqual(
        report_action.tradefed_result_reporters[1].class_name,
        tradefed_config_objects[1].class_name)
